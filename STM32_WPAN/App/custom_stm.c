...
/* WP1625 ESC timing/levels */
#define WP1625_NEUTRAL_US        1500
#define WP1625_DEADBAND_US       30
#define WP1625_REVERSE_CMD_US    1300   /* safe reverse command for WP1625 */
#define WP1625_BRAKE_HOLD_MS     300    /* hold reverse as brake */
#define WP1625_NEUTRAL_HOLD_MS   250    /* then force neutral before arming reverse */
#define WP1625_REVERSE_MIN_US    1200   /* clamp low end for reverse proportional control */
#define WP1625_REVERSE_MAX_US    (WP1625_NEUTRAL_US - WP1625_DEADBAND_US) /* keep margin to neutral */
...
extern uint16_t htim2_speed;
static int reverse_armed = 0;

typedef enum
{
	THR_STATE_PASSTHROUGH = 0,
	THR_STATE_BRAKE,
	THR_STATE_WAIT_NEUTRAL,
	THR_STATE_REVERSE
} throttle_fsm_state_t;

static throttle_fsm_state_t throttle_state = THR_STATE_PASSTHROUGH;
static uint32_t throttle_state_enter_ms = 0;
...
static uint16_t wp1625_reverse_output(uint16_t input_us)
{
	/* Proportional reverse: clamp input within configured reverse window */
	uint16_t clamped = MIN(WP1625_REVERSE_MAX_US, MAX(WP1625_REVERSE_MIN_US, input_us));
	return clamped;
}

static void wp1625_throttle_fsm_step(uint16_t input_us)
{
	uint32_t now = HAL_GetTick();
	uint8_t reverse_req = (input_us + WP1625_DEADBAND_US) < WP1625_NEUTRAL_US;
	uint8_t forward_req = (input_us > (WP1625_NEUTRAL_US + WP1625_DEADBAND_US));

	switch (throttle_state)
	{
		case THR_STATE_PASSTHROUGH:
			if (reverse_req)
			{
				if (reverse_armed)
				{
					throttle_state = THR_STATE_REVERSE;
					setSteeringPWM(2, wp1625_reverse_output(input_us));
				}
				else
				{
					throttle_state = THR_STATE_BRAKE;
					throttle_state_enter_ms = now;
					setSteeringPWM(2, WP1625_REVERSE_CMD_US);
				}
			}
			else
			{
				...
				setSteeringPWM(2, input_us);
			}
			break;

		case THR_STATE_BRAKE:
			/* Hold reverse as brake for a short time */
			setSteeringPWM(2, WP1625_REVERSE_CMD_US);
			if ((now - throttle_state_enter_ms) >= WP1625_BRAKE_HOLD_MS)
			{
				throttle_state = THR_STATE_WAIT_NEUTRAL;
				throttle_state_enter_ms = now;
				setSteeringPWM(2, WP1625_NEUTRAL_US);
			}
			break;

		case THR_STATE_WAIT_NEUTRAL:
			/* Force neutral regardless of input for arming interval */
			setSteeringPWM(2, WP1625_NEUTRAL_US);
			if ((now - throttle_state_enter_ms) >= WP1625_NEUTRAL_HOLD_MS)
			{
				reverse_armed = 1;
				if (reverse_req)
				{
				...
				{
					throttle_state = THR_STATE_PASSTHROUGH;
				}
			}
			break;

		case THR_STATE_REVERSE:
			if (reverse_req)
			{
				setSteeringPWM(2, wp1625_reverse_output(input_us));
			}
			...
			{
				/* Leaving reverse disarms and returns to passthrough */
				reverse_armed = 0;
				throttle_state = THR_STATE_PASSTHROUGH;
				setSteeringPWM(2, input_us);
			}
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomSwitch_CHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            uint8_t val = attribute_modified->Attr_Data[0];
            APP_DBG_MSG("LED WRITE EVENT: handle=0x%04X val=%d\r\n",
                        attribute_modified->Attr_Handle, val);
        ...
        case ACI_GATT_WRITE_PERMIT_REQ_VSEVT_CODE:
          ...
          aci_gatt_write_permit_req_event_rp0 *p_write = (aci_gatt_write_permit_req_event_rp0*)blecore_evt->data;

          if (p_write->Attribute_Handle == CustomContext.CustomLed_CHdle)
          {
        	uint8_t val = p_write->Data[0];
